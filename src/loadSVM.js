...
module.exports = function (libsvm) {
  ...
  const predict_one_probability = libsvm.cwrap(
    'libsvm_predict_one_probability',
    'number',
    ['number', 'array', 'number', 'number'],
  );
  const add_instance = libsvm.cwrap('add_instance', null, [
    'number',
    'array',
    'number',
    'number',
  ...
  const create_svm_nodes = libsvm.cwrap('create_svm_nodes', 'number', [
    'number',
    'number',
  ]);
  const train_problem = libsvm.cwrap('libsvm_train_problem', 'number', [
    'number',
    'string',
  ]);
  const svm_get_nr_sv = libsvm.cwrap('svm_get_nr_sv', 'number', ['number']);
  const svm_get_nr_class = libsvm.cwrap('svm_get_nr_class', 'number', [
    'number',
  ]);
  const svm_get_sv_indices = libsvm.cwrap('svm_get_sv_indices', null, [
    'number',
    'number',
  ]);
  const svm_get_labels = libsvm.cwrap('svm_get_labels', null, [
    'number',
    'number',
  ]);
  const svm_free_model = libsvm.cwrap('svm_free_model', null, ['number']);
  const svm_cross_validation = libsvm.cwrap('libsvm_cross_validation', null, [
    'number',
    'string',
    'number',
    'number',
  ]);
  const svm_get_svr_probability = libsvm.cwrap(
  ...
  const deserialize_model = libsvm.cwrap('deserialize_model', 'number', [
    'string',
  ]);
  ...
  class SVM {
    ...
    train(samples, labels) {
      if (this._deserialized)
        throw new Error(
          'Train cannot be called on instance created with SVM.load',
        );
      this.free();
    ...
    crossValidation(samples, labels, kFold) {
      if (this._deserialized)
        throw new Error(
          'crossValidation cannot be called on instance created with SVM.load',
        );
      ...
      const data = libsvm.HEAPF64.subarray(
        target / 8,
        target / 8 + labels.length,
      );
    ...
    predictOneProbability(sample) {
      ...
      const prediction = predict_one_probability(
        this.model,
        new Uint8Array(new Float64Array(sample).buffer),
        sample.length,
        estimates,
      );
      const estimatesArr = Array.from(
        libsvm.HEAPF64.subarray(estimates / 8, estimates / 8 + nbLabels),
      );
      const result = {
        prediction,
    ...
    _getInterval(confidence) {
      ...
      if (sigma === 0)
        throw new Error(
          'the model is not a regression with probability estimates',
        );
    ...
    getSVIndices() {
      ...
      return getIntArrayFromModel(svm_get_sv_indices, this.model, nSV).map(
        (i) => i - 1,
      );
    }
  ...
  function createProblem(samples, labels) {
    ...
    for (let i = 0; i < nbSamples; i++) {
      add_instance(
        problem,
        new Uint8Array(new Float64Array(samples[i]).buffer),
        nbFeatures,
        labels[i],
        i,
      );
